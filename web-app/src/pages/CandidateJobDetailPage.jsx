...
function CandidateJobDetailPage() {
  ...
  return (
    <main className="workspace-page">
      ...
      <h1>{job.title}</h1>
      <div className="detail-grid"><p><strong>Domain</strong>{job.domain || 'Not specified'}</p><p><strong>Location</strong>{job.location || 'Not specified'}</p><p><strong>Employment</strong>{job.employmentType || 'Not specified'}</p><p><strong>Workplace</strong>{job.workplaceType || 'Not specified'}</p><p><strong>Experience</strong>{job.minimumYearsExperience ?? 0} years</p><p><strong>Education</strong>{job.requiredEducationLevel || 'Not specified'}</p></div>
